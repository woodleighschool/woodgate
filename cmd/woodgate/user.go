...
func createUserCommand(databaseURL *string) *cobra.Command {
	...
	var keys []string
	cmd := &cobra.Command{Use: "create", Short: "Create a user", Args: cobra.NoArgs, RunE: func(cmd *cobra.Command, _ []string) error {
	...
	cmd.Flags().StringSliceVar(&keys, "role", []string{"admin"}, "Role key (repeat for multiple)")
