...
func TestS3MultipartRetryUsesRecordedUploadAndCompletedObject(t *testing.T) {
	const objectPrefix = "test/objects"
	...
	if err := uploads.CompleteMultipart(ctx, object.ID, objectPrefix, parts); err != nil {
	...
	finalized, err := uploads.Finalize(ctx, object.ID, objectPrefix)
	if err != nil {
	...
	if _, err := uploads.Finalize(ctx, object.ID, objectPrefix); err != nil {
