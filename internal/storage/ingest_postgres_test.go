...
func TestDirectUploadTargetsPendingObjectKey(t *testing.T) {
	...
	object, target, err := uploads.BeginDirect(ctx, "test/objects", "Installer.pkg")
	if err != nil {
...
func TestMultipartSigningUsesRecordedObjectUpload(t *testing.T) {
	...
	unrecorded, _, err := uploads.BeginDirect(ctx, "test/objects", "Direct.pkg")
	if err != nil {
		t.Fatalf("begin direct upload: %v", err)
	}
	first, firstAction, err := uploads.Begin(ctx, "test/objects", "First.pkg", 1)
	if err != nil {
	...
	for _, tt := range []struct {
		...
		{name: "wrong prefix", objectID: first.ID, prefix: "checkin/photos"},
	} {
