...
func TestFileStoreDeliversObjectDirectly(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/api/checkin/photos/7/content", nil)
...
func TestFileStorePresignGetProducesBlobCapability(t *testing.T) {
	...
	if got := parsed.Scheme + "://" + parsed.Host + parsed.EscapedPath(); got != "https://woodgate.example/storage/checkin/photos/7/App%20Icon.png" {
	...
	if claims.Key != "checkin/photos/7/App Icon.png" {
...
func TestFileStorePresignPutProducesUploadTarget(t *testing.T) {
	...
	if got := parsed.Scheme + "://" + parsed.Host + parsed.Path; got != "https://woodgate.example/storage/test/objects/42/Installer.pkg" {
	...
	if claims.Key != "test/objects/42/Installer.pkg" {
		t.Fatalf("key = %q, want object key", claims.Key)
	}
	assertCapabilityExpiry(t, claims.Exp, issuedAfter, time.Now())
	if got := store.TransferOrigin(); got != "https://woodgate.example" {
		t.Fatalf("transfer origin = %q, want https://woodgate.example", got)
	}
}
...
func newTestFileStoreAt(t *testing.T, root string) *fileStore {
	t.Helper()
	store, err := newFileStore(root, "https://woodgate.example", testCapabilityKeyHex, testTransferTTL)
	if err != nil {
