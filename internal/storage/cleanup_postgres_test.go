...
func TestUploadCleanupRemovesAbandonedDirectUpload(t *testing.T) {
	...
	object, _, err := ingestor.BeginDirect(ctx, "test/objects", "installer.pkg")
	if err != nil {
...
func TestUploadCleanupRetriesMultipartFailure(t *testing.T) {
	...
	objects := NewObjectStore(db, backend, testLogger())
	ingestor := NewIngestor(objects, backend)
	object, err := objects.CreatePending(ctx, "test/objects", "installer.pkg")
	if err != nil {
...
func TestUploadCleanupClaimsRetriesBeforeNewExpirations(t *testing.T) {
	...
	retry, err := objects.CreatePending(ctx, "test/objects", "retry.pkg")
	if err != nil {
	...
	newlyExpired, err := objects.CreatePending(ctx, "test/objects", "new.pkg")
	if err != nil {
