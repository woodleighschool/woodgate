...
import (
	...
	"github.com/woodleighschool/woodgate/internal/storage/capability"
)

func TestBlobGetServesBytesAndRanges(t *testing.T) {
	t.Parallel()
	store := newTransferTestFileStore(t)
	const key = "test/objects/1/Installer.pkg"
	if err := store.Put(
		t.Context(),
		key,
	...
	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/storage/test/objects/1/Installer.pkg?cap="+token, nil)
...
func TestBlobGetAcceptsEquivalentEscapingForSignedKey(t *testing.T) {
	...
	store := newTransferTestFileStore(t)
	const key = "test/objects/38/Zoom-7.1.5 (84650).pkg"
	...
	req := httptest.NewRequestWithContext(
		t.Context(),
		http.MethodGet,
		"/storage/test/objects/38/Zoom-7.1.5%20(84650).pkg?cap="+token,
		nil,
	)
...
func TestBlobGetRejectsInvalidExpiredAndMissingObjects(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			rec := httptest.NewRecorder()
			req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/storage/checkin/photos/1/icon.png?cap="+tc.cap, nil)
			router.ServeHTTP(rec, req)
			if rec.Code != tc.want {
...
func TestBlobPutWritesAndRejectsWrongOperation(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(t.Context(),
		http.MethodPut,
		"/storage/checkin/photos/7/icon.png?cap="+putToken,
	...
	req = httptest.NewRequestWithContext(t.Context(),
		http.MethodPut,
		"/storage/checkin/photos/7/icon.png?cap="+getToken,
...
func TestBlobRejectsMismatchedPathAndSignedKey(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/storage/checkin/photos/8/icon.png?cap="+token, nil)
	router.ServeHTTP(rec, req)
...
func TestBlobGetLogsOpenFailures(t *testing.T) {
	...
	token := signBlobCapability(t, BlobCapabilityClaims{
		Op:  capability.OpGet,
		Key: "test/objects/1/Installer.pkg",
		Exp: time.Now().Add(time.Minute).Unix(),
	})
...
func TestTransferRoutesAreNotMountedForS3(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/storage/test/objects/1/Installer.pkg", nil)
	router.ServeHTTP(rec, req)
...
func newTransferTestFileStore(t *testing.T) Backend {
	...
	store, err := New(t.Context(), Config{
		...
		File: FileConfig{
			Root:             t.TempDir(),
			BaseURL:          "https://woodgate.example",
