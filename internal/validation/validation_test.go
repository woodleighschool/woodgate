...
func TestStructValidatesFieldsAndHTTPSOrigins(t *testing.T) {
	...
	if err := Struct(input{Name: "woodgate", Origin: "https://example.com/"}); err != nil {
	...
	tests := []struct {
		...
		{
			name: "https",
			in:   input{Name: "woodgate", Origin: "http://example.com"},
			want: "Origin must be an HTTPS origin",
		},
		{
			name: "path",
			in:   input{Name: "woodgate", Origin: "https://example.com/path"},
			want: "Origin must be an HTTPS origin",
		},
	}
