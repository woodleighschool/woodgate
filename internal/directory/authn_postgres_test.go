...
func TestAuthenticatePasswordUsesPersistedCredentials(t *testing.T) {
	database, ctx := testdb.Open(t)
	store := NewStore(database)
	users := NewUserService(store)
	created, err := users.Create(ctx, UserCreate{Email: "admin@example.invalid", Name: "Persisted Admin", Password: "correct-password"})
	if err != nil {
...
func TestRotateAPIKeyReplacesAndRevokesBearerCredential(t *testing.T) {
	...
	store := NewStore(database)
	users := NewUserService(store)
	user, err := users.Create(ctx, UserCreate{Email: "api-user@example.invalid", Name: "API User", Password: "correct-password"})
	if err != nil {
		t.Fatalf("create user: %v", err)
	}
...
func TestSSORejectsLocalOnlyPrincipal(t *testing.T) {
	database, ctx := testdb.Open(t)
	store := NewStore(database)
	if _, err := NewUserService(store).Create(ctx, UserCreate{Email: "local@example.invalid", Password: "correct-password"}); err != nil {
...
func TestAuthnStoreMapsMissingIdentities(t *testing.T) {
	database, ctx := testdb.Open(t)
	store := NewAuthnStore(NewStore(database))
