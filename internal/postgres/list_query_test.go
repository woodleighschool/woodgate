...
func TestListQueryRejectsUnknownSortKey(t *testing.T) {
	_, _, err := ListQuery{
		...
		Params: listing.Normalize(listing.Params{Sort: "station_id"}),
	}.Build()
...
func TestWhereBuilderAddfRegistersArgsInOrder(t *testing.T) {
	var where WhereBuilder
	where.Addf("(name ILIKE %s OR description ILIKE %s)", "%station%", "%station%")
	where.Addf("resource = %s", "stations")
	...
	if len(args) != 3 || args[0] != "%station%" || args[1] != "%station%" || args[2] != "stations" {
		t.Fatalf("args = %#v", args)
	}
}
