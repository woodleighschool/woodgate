...
func (s *Server) connect(w http.ResponseWriter, r *http.Request) {
	...
	protocolVersion, protocolKnown := incompatibleProtocolVersion(protocols)
	...
	if len(protocols) != 1 || protocols[0] != Subprotocol || len(buildHeaders) != 1 || appBuild == "" {
		...
		if err := s.store.observeClient(r.Context(), station.ID, secret, protocolVersion, appBuild); err != nil &&
	...
	if err := s.store.observeClient(r.Context(), station.ID, secret, protocolVersion, appBuild); err != nil {
		s.writeDeviceError(w, r, "connect", err)
		return
	}
	...
	if err := s.hub.serve(r.Context(), ws, station, secret, appBuild); err != nil && !isExpectedClose(err) {
		s.log(r, "connect", err)
...
func incompatibleProtocolVersion(protocols []string) (*int, bool) {
	if len(protocols) != 1 {
		return nil, false
	}
