...
func (f *deviceStoreFake) ObserveRejectedClient(
	_ context.Context,
	_ int64,
	secret string,
	version *int,
...
func (f *deviceStoreFake) claimClientSession(
	_ context.Context,
	_ int64,
	...
	f.mu.Lock()
	defer f.mu.Unlock()
	f.sessionID = connectionID
	f.sessionVersion = version
	f.sessionBuild = build
	return nil
}

func (f *deviceStoreFake) renewClientSession(_ context.Context, _ int64, connectionID string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	if connectionID == "" || connectionID != f.sessionID {
		return ErrSessionInvalid
	}
	return nil
}

func (f *deviceStoreFake) releaseClientSession(_ context.Context, _ int64, connectionID string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	if connectionID == f.sessionID {
		f.sessionID = ""
	}
	return nil
}
