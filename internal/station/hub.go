...
type deviceStore interface {
	...
	ObserveRejectedClient(context.Context, int64, string, *int, string) error
	claimClientSession(context.Context, int64, string, string, int, string) error
	renewClientSession(context.Context, int64, string) error
	releaseClientSession(context.Context, int64, string) error
}

// Hub owns this process's sockets; PostgreSQL owns Station session authority.
type Hub struct {
	store  deviceStore
	logger *slog.Logger
	build  string
...
func (h *Hub) serve(
	...
	if err := h.store.claimClientSession(
		ctx, station.ID, secret, connectionID, protocolVersion, version,
	); err != nil {
		_ = ws.Close(websocket.StatusPolicyViolation, "station session rejected")
		return err
	}
	...
	if !h.register(connection) {
		releaseCtx, releaseCancel := context.WithTimeout(context.Background(), presenceTimeout)
		_ = h.store.releaseClientSession(releaseCtx, station.ID, connectionID)
		releaseCancel()
		return errHubClosed
	}
...
func (h *Hub) readLoop(
	...
	for {
		...
		if err := h.renew(ctx, connection); err != nil {
			_ = ws.Close(websocket.StatusPolicyViolation, "station session changed")
			return err
		}
	}
}
...
func (h *Hub) writeLoop(
	...
	for {
		...
		case <-ticker.C:
			...
			err := h.renew(pingCtx, connection)
			if err == nil {
				err = connection.ws.Ping(pingCtx)
			}
			pingCancel()
			if err != nil {
				if !errors.Is(err, ErrSessionInvalid) && ctx.Err() == nil {
...
func (h *Hub) renew(ctx context.Context, connection *controlConnection) error {
	return h.store.renewClientSession(ctx, connection.stationID, connection.id)
}
...
func (h *Hub) unregister(connection *controlConnection) {
	...
	h.mu.Unlock()
	releaseCtx, cancel := context.WithTimeout(context.Background(), presenceTimeout)
	defer cancel()
	if err := h.store.releaseClientSession(releaseCtx, connection.stationID, connection.id); err != nil {
		h.logger.WarnContext(releaseCtx, "station session release failed",
			"station_id", connection.stationID,
			"err", err,
		)
	}
