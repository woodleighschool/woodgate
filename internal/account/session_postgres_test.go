...
func TestAccountCredentialsAndAdmission(t *testing.T) { //nolint:funlen // Keep credential replacement and access revocation in one ordered HTTP lifecycle.
	...
	// Logout cannot depend on either current admission or a working grants store.
	...
	if _, err := database.Exec(ctx, `INSERT INTO authz_user_roles(user_id,role_id) VALUES ($1,$2)`, user.ID, adminID); err != nil {
		t.Fatal(err)
	}
