...
func TestDatabaseURLsPreserveConnectionSettings(t *testing.T) {
	...
	const base = "postgres://woodgate:secret@database.test:5432/custom?sslmode=disable&pool_max_conns=12" //nolint:gosec // Fixed test database credential.
	adminURL, databaseURL, err := databaseURLs(base, "woodgate_test_example")
	if err != nil {
	...
	for name, rawURL := range map[string]string{"admin": adminURL, "database": databaseURL} {
		...
		if parsed.User.String() != "woodgate:secret" ||
	...
	if parsed, _ := url.Parse(databaseURL); parsed.Path != "/woodgate_test_example" {
		t.Fatalf("isolated database path = %q, want /woodgate_test_example", parsed.Path)
	}
}
...
func TestDatabaseURLsRejectUnsupportedScheme(t *testing.T) {
	...
	if _, _, err := databaseURLs("mysql://database.test/woodgate", "woodgate_test_example"); err == nil {
