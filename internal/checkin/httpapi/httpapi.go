...
func RegisterAPI(routes api.AppRoutes, deps Dependencies) {
	...
	locationContent := routes.Transfers.With(
		authhttp.RequireAuth(deps.Authenticator, deps.Logger),
		authhttp.RequirePermission(deps.Authorizer, deps.Logger, rbac.ResourceLocations, authz.View),
	)
	...
	checkinContent := routes.Transfers.With(
		authhttp.RequireAuth(deps.Authenticator, deps.Logger),
		authhttp.RequirePermission(deps.Authorizer, deps.Logger, rbac.ResourceCheckins, authz.View),
	)
...
type locationGroupListInput struct{ api.ListQueryInput }
type locationListOutput struct{ Body api.Page[checkin.Location] }
type locationGroupListOutput struct {
	Body api.Page[directory.GroupSummary]
}
...
func registerLocations(routes huma.API, deps Dependencies) {
	huma.Register(routes, authhuma.Require(routes, deps.Authorizer, deps.Logger, rbac.ResourceLocations, authz.View, huma.Operation{
	...
	huma.Register(routes, authhuma.Require(routes, deps.Authorizer, deps.Logger, rbac.ResourceLocations, authz.Edit, huma.Operation{
		OperationID: "list-location-groups", Method: http.MethodGet, Path: "/api/locations/groups", Tags: []string{api.TagLocations}, Summary: "List location group choices",
	}), func(ctx context.Context, input *locationGroupListInput) (*locationGroupListOutput, error) {
		items, count, err := deps.Service.ListLocationGroupChoices(ctx, input.Params())
		if err != nil {
			return nil, api.ResourceError(ctx, deps.Logger, "list-location-groups", "directory group", err)
		}
		return &locationGroupListOutput{Body: api.Page[directory.GroupSummary]{Items: items, Count: count}}, nil
	})
...
func registerCheckins(routes huma.API, deps Dependencies) {
	...
	huma.Register(routes, authhuma.Require(routes, deps.Authorizer, deps.Logger, rbac.ResourceCheckins, authz.View, huma.Operation{OperationID: "get-checkin", Method: http.MethodGet, Path: checkinPath, Tags: []string{api.TagCheckins}, Summary: "Get a check-in"}), func(ctx context.Context, input *checkinIDInput) (*checkinOutput, error) {
...
func registerLocationAttachment(routes huma.API, deps Dependencies, registration locationAttachmentRegistration) {
	...
	huma.Register(routes, authhuma.Require(routes, deps.Authorizer, deps.Logger, rbac.ResourceLocations, authz.Edit, huma.Operation{
		...
		return &directUploadOutput{Body: directUploadTarget{ObjectID: object.ID, Upload: blobyhuma.UploadAction(target)}}, nil
	})
...
func newAttachmentObjectView(object bloby.Object, basePath string) attachmentObjectView {
