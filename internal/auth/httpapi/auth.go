...
// PermissionDirectory returns a user's effective authorization grants.
type PermissionDirectory interface {
...
func registerOperations(routes api.AppRoutes, deps Dependencies) {
	...
	registerGetAccount(routes.Protected, deps.AuthService, deps.Permissions, deps.Logger)
	registerPutAccount(routes.Protected, deps.Users, deps.Permissions, deps.Logger)
}
