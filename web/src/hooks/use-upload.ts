...
import { type DirectUploadRequest, type UploadProgress, uploadDirect } from "@lib/upload";

interface UploadOptions<TIntent, TResult, TVars extends { file: File }> {
  mutationKey: MutationKey;
  createIntent: (vars: TVars) => Promise<TIntent>;
  uploadRequest: (intent: TIntent, vars: TVars) => DirectUploadRequest;
  ...
  const mutation = useMutation<TResult, Error, TVars>({
    ...
    mutationFn: async (vars) => {
      ...
      try {
        ...
        await uploadDirect({
          request: uploadRequest(intent, vars),
          file: vars.file,
