...
import { useUserRoles, useUsers } from "@features/directory/users/queries";
import type { User } from "@lib/api";
import { DEFAULT_PAGE_SIZE } from "@lib/pagination";
import { nonEmpty } from "@lib/utils";

const routeApi = getRouteApi("/_authenticated/directory/users/");
const USER_FILTER_KEYS = [{ id: "source" }, { id: "role", multiple: true }] as const;
...
const userColumns: DataTableColumnDef<UserTableRow>[] = [
  ...
  {
    id: "role",
    accessorFn: (row) => row.user.effective_roles.map((role) => String(role.id)),
    header: "Roles",
    cell: ({ row }) => <EffectiveRoles roles={row.original.user.effective_roles} />,
    enableSorting: false,
...
export function UserListPage() {
  ...
  const tableRows = React.useMemo<UserTableRow[]>(
    ...
    [currentUser?.id, query.data?.items],
  );
  const totalCount = query.data?.count ?? 0;
  const pageCount = query.data ? Math.ceil(totalCount / tableSearch.per_page) : -1;
  const groupLabel =
    groupID === undefined ? undefined : (group.data?.display_name ?? "Selected group");
