...
export function RoleEditPage() {
  ...
  const { id: rawID } = useParams({ from: "/_authenticated/access/roles/$id/edit" });
  ...
  return (
    <RoleForm
      ...
      onSubmit={(body) => {
        void update
          .mutateAsync(body)
          .then((role) => navigate({ to: "/access/roles/$id", params: { id: String(role.id) } }));
      }}
      onCancel={() => void navigate({ to: "/access/roles/$id", params: { id: String(id) } })}
    />
  );
}
