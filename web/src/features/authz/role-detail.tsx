...
import { canAccess, permissionLevel } from "@features/authz/permissions";
...
export function RoleDetailPage() {
  ...
  const { id: rawID } = useParams({ from: "/_authenticated/access/roles/$id" });
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        actions={
          !role.data.builtin && canAccess(account.data, "authz.roles", "edit") ? (
            <>
              <Button
                size="sm"
                variant="outline"
                render={<Link to="/access/roles/$id/edit" params={{ id: String(role.data.id) }} />}
                nativeButton={false}
              >
      ...
      <ConfirmDialog
        ...
        title="Delete Role?"
        description="Assignments to this role will also be removed."
        confirmLabel="Delete"
        pending={remove.isPending}
        onConfirm={() => {
          void remove.mutateAsync(role.data.id).then(() => navigate({ to: "/access/roles" }));
        }}
      />
    </PageShell>
  );
